...
public class Empresa {
    ...
    public void obtenerMasaSalarial() {
        double total=0;
        for (int i=0;i<contadortrabajadores;i++) {
            if (ListaTrabajadores[i].isEnactivo()) {//Error: esta negando
                total+=ListaTrabajadores[i].getSalario();//Error: debe sumar
            }
        }
        System.out.println("===================================================================================");
    ...
    public boolean darDeBaja(String nom, String ap) {
        ...
        if (trab!=-1) {
            for (int i=0;i<contadortrabajadores;i++) {
                if (ListaTrabajadores[i].getId()==trab) {
                    ListaTrabajadores[i].setEnactivo(false);//Error: debe ser false
                    return true;
                }
            }
        }
